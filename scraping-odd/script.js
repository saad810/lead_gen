...
async function scrapeMC(page, mcNumber) {
  ...
  try {
    ...
    await page.waitForSelector('input[name="query_param"][value="MC_MX"]', { timeout: 15000 });
    await page.waitForSelector('input[name="query_string"]', { timeout: 15000 });
...
// let shuttingDown = false;
async function handleCrash(files) {
  try {
    const lastFile = files[files.length - 1];
    const endtime = Date.now();
    const zipPath = await createZipBatch(files, Math.floor(lastFile / BATCH_SIZE));
    await uploadToR2(zipPath, path.basename(zipPath));
    await delay(2000);
    logToFile(TIME_RECORDS_FILE, `${id}\tZip ${zipPath} Created at: ${new Date(endtime).toISOString()}`);

    lastMCSaved = lastFile;
    await saveLastZippedMC(lastMCSaved);
    await deleteFiles(files);
    console.log("Crash files handled successfully.");

  } catch {
    console.error("Error handling crash files.");
  }
}
async function shutdownHandler(reason) {
  if (shuttingDown) return;
  shuttingDown = true;

  try {
    console.log(reason, "handler started");
    const stopTime = new Date().toISOString();
    logToFile(TIME_RECORDS_FILE, `${id}\tStopped: ${reason} at ${stopTime}`);
    if (reason === "uncaughtException" || reason === "unhandledRejection") {
      await delay(5000);
      if (savedCount > 0) {
        await handleCrash(savedFiles);  // await ensures zip & upload complete
      }
    }
    ...
    logToFile(ERR_LOG_FILE, `${id}\tShutdown error: ${err.stack}`);
  } finally {
    console.log("Exiting process now.");
    process.exit(0);  // only exit after everything finishes
  }
}
